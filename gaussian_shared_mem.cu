...
/**
 * @brief CUDA 核函数：使用共享内存瓦片执行消元步骤，并融合了置零操作
 * @tparam BLOCK_DIM 线程块的维度，必须在编译时确定
 */
template <int BLOCK_DIM>
__global__ void elimination_kernel_shared(float *d_m, int n, int k) {
    // 定义共享内存瓦片
    __shared__ float pivot_tile[BLOCK_DIM];
    __shared__ float factor_tile[BLOCK_DIM];

    // 计算全局行和列索引
    int row = k + 1 + blockIdx.y * BLOCK_DIM + threadIdx.y;
    int col = k + 1 + blockIdx.x * BLOCK_DIM + threadIdx.x;

    // 1. 从全局内存加载数据到共享内存
    // 每个线程加载一个主元行元素和一个因子列元素
    if (threadIdx.x < BLOCK_DIM && (k + 1 + blockIdx.x * BLOCK_DIM + threadIdx.x) <= n) {
        pivot_tile[threadIdx.x] = d_m[k * (n + 1) + (k + 1 + blockIdx.x * BLOCK_DIM + threadIdx.x)];
    }
    if (threadIdx.y < BLOCK_DIM && (k + 1 + blockIdx.y * BLOCK_DIM + threadIdx.y) < n) {
        factor_tile[threadIdx.y] = d_m[(k + 1 + blockIdx.y * BLOCK_DIM + threadIdx.y) * (n + 1) + k];
    }
    ...
    if (row < n && col <= n) {
        ...
        float pivot_val = pivot_tile[threadIdx.x];
    ...
    // 3. 内核融合：由块内y维度上的第一个线程将 A[i][k] 置为 0
...
void launch_elimination_kernel(float* d_m, int n, int k, int block_dim) {
    ...
    dim3 grid(
        (n + 1 - (k + 1) + block.x - 1) / block.x,
        (n - (k + 1) + block.y - 1) / block.y
    );

    switch (block_dim) {
        case 8:
            elimination_kernel_shared<8><<<grid, block>>>(d_m, n, k);
            break;
        case 16:
            elimination_kernel_shared<16><<<grid, block>>>(d_m, n, k);
            break;
        case 32:
            elimination_kernel_shared<32><<<grid, block>>>(d_m, n, k);
            break;
        default:
