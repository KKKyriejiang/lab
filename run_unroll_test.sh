...
# 清理旧文件
rm -f gaussian_unroll results_unroll.csv

# 编译使用循环展开的 CUDA 代码
echo "Compiling gaussian_unroll.cu..."
nvcc gaussian_unroll.cu -o gaussian_unroll
if [ $? -ne 0 ]; then
...
for N in "${SIZES[@]}"; do
    for B_DIM in "${BLOCK_DIMS[@]}"; do
        echo "-------------------------------------"
        echo "Running Unroll version for N = $N with BlockDim = $B_DIM"
        
        # 运行程序并捕获输出
        output=$(./gaussian_unroll $N $B_DIM)
...
echo "-------------------------------------"
echo "Data collection for loop unrolling tests finished."
