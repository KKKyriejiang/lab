...
# 清理旧文件
rm -f gaussian_shared_mem results_shared_mem.csv

# 编译使用共享内存的 CUDA 代码
echo "Compiling gaussian_shared_mem.cu..."
nvcc gaussian_shared_mem.cu -o gaussian_shared_mem
if [ $? -ne 0 ]; then
...
DATA_FILE="results_shared_mem.csv"
...
for N in "${SIZES[@]}"; do
    for B_DIM in "${BLOCK_DIMS[@]}"; do
        echo "-------------------------------------"
        echo "Running Shared Memory version for N = $N with BlockDim = $B_DIM"
        
        # 运行程序并捕获输出
        output=$(./gaussian_shared_mem $N $B_DIM)
...
echo "-------------------------------------"
echo "Data collection for shared memory tests finished."
